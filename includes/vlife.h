...
public:
    ...
    int isGlider(VersaTile* sqt, int px, int py, bool nuke, urow_t* cachearray) {

        if (cachearray[py] & (1ull << ((BITTAGE - 1) - px)))
            return 2;

        if ((px < 4) || (py < 4) || (px > (BITTAGE - 5)) || (py > TVSPACE - 3))
            return 0;
        ...
        if ((sqt->d[y-2] | sqt->d[y+2]) & (31ull << ((BITTAGE - 3) - x)))
            return 0;

        int projection = ((sqt->d[y+1] | sqt->d[y] | sqt->d[y-1]) >> ((BITTAGE - 3) - x)) & 31ull;
        ...
        if ((x < 5) || (x > (BITTAGE - 6))) {
            return 0;
        } else if ((sqt->d[y] | sqt->d[y-1] | sqt->d[y+1]) & (99ull << (THSPACE - x))) {
            return 0;
        } else if ((sqt->d[y-2] | sqt->d[y+2]) & (127ull << (THSPACE - x))) {
            return 0;
        } else if ((sqt->d[y-3] | sqt->d[y+3]) & (60ull << (THSPACE - x))) {
            return 0;
        } else {
            ...
            int high3 = ((sqt->d[y]) >> ((BITTAGE - 2) - x)) & 7ull;
            int low6 = (((sqt->d[y-1]) >> ((BITTAGE - 2) - x)) & 7ull) | (((sqt->d[y+1]) >> ((BITTAGE - 5) - x)) & 56ull);

            if (array[high3] & (1ull << low6)) {

                cachearray[y-1] |= (7ull << ((BITTAGE - 2) - x));
                cachearray[y] |= (7ull << ((BITTAGE - 2) - x));
                cachearray[y+1] |= (7ull << ((BITTAGE - 2) - x));
    ...
    int isBlinker(VersaTile* sqt, int x, int y) {
        if ((x < 5) || (y < 5) || (x > (BITTAGE - 6)) || (y > TVSPACE - 2)) {
            return 0;
        } else if ((sqt->d[y] | sqt->hist[y]) & (99ull << (THSPACE - x))) {
            return 0;
        } else if ((sqt->d[y-1] | sqt->hist[y-1] | sqt->d[y+1] | sqt->hist[y+1]) & (54ull << (THSPACE - x))) {
            return 0;
        } else if ((sqt->d[y-2] | sqt->hist[y-2] | sqt->d[y+2] | sqt->hist[y+2]) & (62ull << (THSPACE - x))) {
            return 0;
        } else if ((sqt->d[y-3] | sqt->hist[y-3] | sqt->d[y+3] | sqt->hist[y+3]) & (8ull << (THSPACE - x))) {
            return 0;
        } else {
    ...
    int isVerticalBeehive(VersaTile* sqt, int x, int y) {
        if ((x < 5) || (y < 4) || (x > (BITTAGE - 6)) || (y > TVSPACE - 4)) {
            return 0;
        } else if ((sqt->d[y+1] | sqt->hist[y+1] | sqt->d[y+2] | sqt->hist[y+2]) & (107ull << (THSPACE - x))) {
            return 0;
        } else if ((sqt->d[y] | sqt->hist[y] | sqt->d[y+3] | sqt->hist[y+3]) & (119ull << (THSPACE - x))) {
            return 0;
        } else if ((sqt->d[y-1] | sqt->hist[y-1] | sqt->d[y+4] | sqt->hist[y+4]) & (62ull << (THSPACE - x))) {
            return 0;
        } else if ((sqt->d[y-2] | sqt->hist[y-2] | sqt->d[y+5] | sqt->hist[y+5]) & (8ull << (THSPACE - x))) {
            return 0;
        } else {
            setcell(sqt, x, y, 0, true);
    ...
    int isAnnoyance(VersaTile* sqt, int x, int y) {

        return 0;

        if ((x < 4) || (y < 4) || (x > (THSPACE - 6)) || (y > TVSPACE - 2)) {
            return 0;
        } else {
            if ((sqt->d[y] >> ((BITTAGE - 2) - x)) & 1) {
                if ((sqt->d[y+1] >> ((BITTAGE - 1) - x)) & 1) {
                    if ((sqt->d[y] | sqt->hist[y] | sqt->d[y+1] | sqt->hist[y+1]) & (51ull << (THSPACE - x))) {
                        return 0;
                    } else if ((sqt->d[y-1] | sqt->hist[y-1] | sqt->d[y+2] | sqt->hist[y+2]) & (63ull << (THSPACE - x))) {
                        return 0;
                    } else if ((sqt->d[y-2] | sqt->hist[y-2] | sqt->d[y+3] | sqt->hist[y+3]) & (30ull << (THSPACE - x))) {
                        return 0;
                    } else {
